...
class CustomEventGenerator:
    ...
    def generate_event(self) -> Dict[str, Any]:
        ...
        event.update({
                    ...
                    'true_churn_label': str(self.labels.iloc[idx]) if self.labels is not None else None 
                    })
        return event
...
class MLKafkaProducer:
    ...
    def _log_event(self, event: Dict[str, Any], success: bool, count: int):
        ...
        senior_citizen = str(event.get('SeniorCitizen', 'N/A'))[:5]
        gender = str(event.get('gender', 'N/A'))[:2]
        
        print(f"{status} Event {count:3d}: Customer {customer_id} | {senior_citizen} | {gender}")
